...
class Pos extends MY_Controller {
    ...
    function index($sid = NULL, $eid = NULL) {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($customer_details->id == 19999999 && $this->tec->formatDecimal($paid) < $this->tec->formatDecimal($round_total)) {
    ...
    function amount() {
            $customer = $this->input->get('custom');
            $start_date = $this->input->get('start_date') ? $this->input->get('start_date') : NULL;
            $end_date = $this->input->get('end_date') ? $this->input->get('end_date') : NULL;
            $user = $this->input->get('user') ? $this->input->get('user') : NULL;
            $this->data['total_sale'] = $this->pos_model->getTotalCustomerSales($customer, $start_date, $end_date, $user);
            
            $data = json_encode($this->data['total_sale']);
            echo $data;
            exit;
    }
    ...
    function register_details() {
        ...
        $this->data['due'] = $this->pos_model->getRegisterStatus($register_open_time);
        $this->data['expenses'] = $this->pos_model->getRegisterExpenses($register_open_time);
        var_dump( $this->data['other_sales']);
